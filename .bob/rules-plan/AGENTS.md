...
- **Single dispatcher pattern**: `edu.java.JavaOpenAI.process()` is the only command router — new examples register a CLI command string there via a `switch` case; do not create separate entry points
- **`main()` constraint**: `main()` is the only `static` method allowed in `JavaOpenAI`; all other methods must be instance methods — this is an explicit project rule
...
- **Cloud-only examples**: TTS, STT, image generation, and moderation are cloud-only by design — do not plan local-server support for these
- **No separate test utilities directory**: `TestConfig.java` lives in `src/test/java/edu/java/` alongside the test class — keep test utilities there
...
- **Version sync**: `JavaOpenAI.JAVAOPENAI_VERSION` and `pom.xml` version must be kept in sync manually — there is no build-time substitution
